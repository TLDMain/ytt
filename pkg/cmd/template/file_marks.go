...
func (s *FileMarksOpts) Set(cmdFlags CmdFlags) {
	...
	cmdFlags.BoolVar(&s.StarStrict, "file-mark-strict", true, "Error if a file path pattern containing '*' matches no files")
}
...
func (s *FileMarksOpts) Apply(filesToProcess []*files.File) ([]*files.File, error) {
	...
	for _, mark := range s.FileMarks {
		...
		if !matched && (s.StarStrict || !strings.Contains(path, "*")) {
